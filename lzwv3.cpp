...
#include <iterator>
#include <string.h>
#include <filesystem> //Libreria para operar sobre sistema de archivos y componentes 
...
//Verifica si lo que se digito en fichero_entrada es un archivo o directorio,devuelve 0 si es un directorio
            if(!std::filesystem::is_regular_file(fichero_entrada)) 
            {
                for(const auto & entry: fs::directory_iterator(fichero_entrada)) //Itera sobre los archivos que hay en dicho directorio
